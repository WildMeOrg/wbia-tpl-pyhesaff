...
def extract_2darr_list(size_list, ptr_list, arr_t, arr_dtype, arr_dim):
    """
    ...
    """
    iter_ = ((arr_ptr, (size, arr_dim)) for (arr_ptr, size) in zip(ptr_list, size_list))
    arr_list = [
        arrptr_to_np(arr_ptr, shape, arr_t, arr_dtype) for arr_ptr, shape in iter_
    ]
    return arr_list
